...
async def savegroup_and_welcome(bot, message):
    ...
    if bot.id in r_j_check:
        ...
        if message.chat.id in temp.BANNED_CHATS:
            ...
            k = await message.reply("CHAT NON AUTORISÉ 🐞\n\nMon administrateur m'a empêché de travailler ici ! Si vous souhaitez en savoir plus, contactez le support", reply_markup=InlineKeyboardMarkup(buttons))
            try: await k.pin()
            except: pass
            return await bot.leave_chat(message.chat.id)
           
        buttons = [[InlineKeyboardButton('Hᴇʟᴩ', url=f"https://t.me/{temp.U_NAME}?start=help")]]
        await message.reply(text="❤️ MERCI DE M'AJOUTER À VOTRE GROUPE.\n» N'OUBLIEZ PAS DE ME FAIRE ADMINISTRATEUR.\n» EN CAS DE DOUTE SUR MON UTILISATION, CLIQUEZ SUR LE BOUTON CI-DESSOUS...✨", reply_markup=InlineKeyboardMarkup(buttons))
    else:
...
async def leave_a_chat(bot, message):
    ...
    try:
        buttons = [[InlineKeyboardButton('Sᴜᴩᴩᴏʀᴛ', url=f'https://t.me/{SUPPORT_CHAT}')]]
        await bot.send_message(chat_id=chat, text='<b>Hello Friends, \nMon administrateur m'a dit de quitter le groupe alors j'y vais ! Si vous souhaitez m'ajouter à nouveau, contactez mon groupe de support.</b>', reply_markup=InlineKeyboardMarkup(buttons))
